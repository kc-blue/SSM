    		...
    		<form action="${pageContext.request.contextPath }/items/addCar.action" method="post">
    			<table class="table table-bordered table-hover">
		    		...
		    		<tr align="center">
			    		<td>${item.name }</td>
			    		<td>${item.price }</td>
			    		<td><img title="${item.detail }" style="width:30px;height:30px" src="${pageContext.request.contextPath }/upload/${item.pic}"></td>
			    		<td>${item.detail }</td>
			    		<td><fmt:formatDate value="${item.createtime}" pattern="yyyy-MM-dd"/></td>
			    		<td><a href="${pageContext.request.contextPath }/items/del.action?id=${item.id}"><button type="button" class="btn btn-success btn-lg" onclick="return confirm('确认要删除信息') "><span class="glyphicon glyphicon-trash"></span> 删除</button></a></td>
	    				<td><a href="${pageContext.request.contextPath }/items/findOne.action?id=${item.id}"><button type="button" class="btn btn-success btn-lg"><span class="glyphicon glyphicon-edit"></span> 修改</button></a></td>    				
    				</tr>
